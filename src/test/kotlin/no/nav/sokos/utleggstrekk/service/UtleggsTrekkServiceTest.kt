        ...
        Given("hentOgSendUtleggstrekk initieres") {
            ...
            val utleggsTrekkService =
                UtleggstrekkService(
                    ...
                    skeClient =
                        mockk<SkeClient> {
                            coEvery { hentAlleUtleggstrekk() } returns jsonConfig.decodeFromString<List<Trekkpaalegg>>(fileAsString("FraSkatt_Trekkversjon1_1Trekkalternativ-2trekk.json"))
                        },
            ...
            then("Deretter skal trekk som er laget i behandleTrekkservice sendes og status skal oppdateres") {
                val trekkIdatabase = databaseService.hentAlleTrekkSomIkkeErSendt()
                val trekkPairs = jsonConfig.decodeFromString<List<Pair<UtleggstrekkTable, List<TrekkTilOppdrag>>>>(fileAsString("TrekkTilSendingPairMedRiktigCorrid.json"))
